import streamlit as st
from utils import auth as _auth; _auth.login_gate()
from utils.theme import apply_theme
apply_theme()
from utils.excel_loader import load_all
from utils.tab_builders import build_financials
from utils.exporters import render_export_buttons
from utils.fy_filter import fy_filter, apply_fy_filter

st.title("💰 Financial Intelligence — CAPEX/OPEX + EVM")

data = load_all()

c1, _ = st.columns([2, 6])
with c1:
    fy_sel = fy_filter(key="fy_filter_fin", label="Financial Year")

if fy_sel:
    data = dict(data)
    data["projects"] = apply_fy_filter(data.get("projects"), fy_sel)
    data["financials"] = apply_fy_filter(data.get("financials"), fy_sel)
    data["costbenefit"] = apply_fy_filter(data.get("costbenefit"), fy_sel)

bundle = build_financials(data)
if bundle["kpis"]:
    k = st.columns(len(bundle["kpis"]))
    for col,(l,v) in zip(k, bundle["kpis"]): col.metric(l, v)
for _, fig in bundle["figs"]: st.plotly_chart(fig, use_container_width=True)
for name, df in bundle["tables"]:
    st.dataframe(df, use_container_width=True, hide_index=True)
render_export_buttons(bundle)
