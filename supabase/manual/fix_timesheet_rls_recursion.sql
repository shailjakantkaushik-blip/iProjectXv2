...
-- Fixes: infinite recursion detected in policy for relation "timesheets"
--
-- Cause: timesheets SELECT looked at timesheet_entries, and entries SELECT
-- looked back at timesheets → RLS recursion.
-- Fix: SECURITY DEFINER helpers for cross-table checks.
