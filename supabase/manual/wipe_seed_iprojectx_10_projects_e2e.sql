-- =========================================================================
-- iProjectX org ONLY — Wipe + seed 10 end-to-end portfolio projects
...
--
-- SEEDS
--   10 projects with Core + second stream, method-specific stage gates when uses_stage_gates (Agile uses Agile template),
...
DECLARE
  ...
  i int;
  j int;
  codes text[] := ARRAY['PRJ-001','PRJ-002','PRJ-003','PRJ-004','PRJ-005','PRJ-006','PRJ-007','PRJ-008','PRJ-009','PRJ-010'];
  names text[] := ARRAY[
    'Customer Portal Redesign','Core Banking API Platform',
    'Data Lakehouse Foundation','Cyber Resilience Uplift',
    'Contact Centre Omnichannel','Finance Close Automation',
    'HR Self-Service Suite','Supplier Portal 2.0',
    'Branch Network WiFi Refresh','Regulatory Reporting Engine'
  ];
  programs text[] := ARRAY['Digital Transformation','Platform Modernisation','Data & Analytics','Risk & Compliance','Customer Experience','Finance Transformation','People Systems','Procurement','Infrastructure','Risk & Compliance'];
  ...
  statuses public.project_status[] := ARRAY['In Progress','In Progress','In Progress','In Progress','In Progress','In Progress','In Progress','In Progress','In Progress','In Progress'];
  rags public.project_rag[] := ARRAY['Green','Amber','Green','Amber','Green','Green','Amber','Green','Red','Amber'];
  priorities text[] := ARRAY['P1 - Critical','P1 - Critical','P2 - High','P1 - Critical','P2 - High','P2 - High','P3 - Medium','P3 - Medium','P2 - High','P1 - Critical'];
  methods text[] := ARRAY['Hybrid','Agile','Waterfall','Hybrid','Agile','Waterfall','Hybrid','Agile','Waterfall','Hybrid'];
  budgets numeric[] := ARRAY[3200000,5800000,4100000,2700000,1900000,1500000,1200000,980000,2200000,3600000];
  capex_a numeric[] := ARRAY[2500000,4800000,3500000,2000000,1400000,1100000,900000,750000,2000000,2900000];
  capex_i numeric[] := ARRAY[1100000,3100000,900000,400000,1250000,1050000,450000,80000,1600000,1400000];
  opex_a numeric[] := ARRAY[700000,1000000,600000,700000,500000,400000,300000,230000,200000,700000];
  opex_i numeric[] := ARRAY[280000,620000,150000,180000,410000,360000,120000,20000,150000,300000];
  facs numeric[] := ARRAY[3300000,6100000,4200000,2850000,1950000,1520000,1280000,1000000,2550000,3750000];
  ben_t numeric[] := ARRAY[5200000,9500000,7000000,3500000,3100000,2400000,1800000,1600000,1800000,4200000];
  ben_r numeric[] := ARRAY[900000,1200000,200000,0,1800000,1600000,150000,0,200000,400000];
  rois numeric[] := ARRAY[62.5,63.8,70.7,29.6,63.2,60,50,63.3,-18.2,16.7];
  starts date[] := ARRAY['2025-04-01','2024-10-01','2025-07-01','2025-11-01','2024-08-01','2024-05-01','2025-06-01','2026-01-15','2025-02-01','2025-03-01'];
  ends date[] := ARRAY['2026-09-30','2026-06-30','2027-03-31','2026-12-31','2026-04-30','2026-02-28','2026-08-31','2026-12-15','2026-05-31','2026-11-30'];
  lives date[] := ARRAY['2026-08-15','2026-05-01','2027-01-15','2026-11-30','2026-03-15','2026-01-20','2026-07-15','2026-11-01','2026-04-30','2026-10-15'];
  sponsor_titles text[] := ARRAY['CDO','CTO','CDO','CISO','COO','CFO','CHRO','CPO','CTO','CRO'];
  alt_names text[] := ARRAY['Experience','Platform','Data','Security','Omnichannel','Automation','Self-Service','Procurement','Network','Reporting'];
  alt_codes text[] := ARRAY['XP','PLT','DATA','SEC','OMNI','AUTO','SS','PROC','NET','REP'];
  align text[] := ARRAY['Customer Experience','Digital Transformation','Growth','Risk Reduction','Customer Experience','Efficiency','Efficiency','Digital Transformation','Cost Optimisation','Compliance'];
...
-- Verification — iProjectX only (expect 10 projects, 20 streams, ~180 gates)
...
-- iProjectX should show projects = 10.
