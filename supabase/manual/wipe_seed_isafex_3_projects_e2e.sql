-- =========================================================================
-- iSafeX org ONLY — Wipe + seed 3 end-to-end portfolio projects
-- Paste into Supabase SQL Editor and run once.
--
-- SCOPE
--   Targets organization slug = 'isafex' (name iSafeX).
--   Does NOT touch iProjectX or any other organisation.
--
-- KEEPS (control plane / other orgs)
--   All iProjectX data unchanged
--   organizations, profiles, user_roles, business_units (for all orgs)
--   billing / landing / invoice template config
--
-- WIPES then RESEEDS (iSafeX only)
--   projects (+ cascaded children: streams, gates, milestones, RAID, CRs, …)
--   resources / allocations for iSafeX
--   financials_monthly, fy_allocations, benefits, opex_other_costs
--   work_items, sprints, timesheets, demand_pipeline, scenarios
...
--
-- SEEDS
--   3 projects (SAFE-001…003) with Core + second stream, 9 stage gates/stream,
...
--
-- Org admin expected: shailja.kant.kaushik@gmail.com
-- Requires: always-on Core streams + timesheet / work-item migrations.
-- =========================================================================

BEGIN;

-- Resolve iSafeX org (fail closed — never wipe other tenants)
DO $resolve$
DECLARE
  v_org uuid;
BEGIN
  ...
  WHERE lower(slug) = 'isafex'
     OR lower(name) = 'isafex'
  ORDER BY CASE WHEN lower(slug) = 'isafex' THEN 0 ELSE 1 END
  LIMIT 1;

  IF v_org IS NULL THEN
    RAISE EXCEPTION 'iSafeX organisation not found (slug=isafex). Aborting — other orgs left untouched.';
  END IF;

  PERFORM set_config('isafex.seed_org_id', v_org::text, true);
  RAISE NOTICE 'Seeding iSafeX org % only — other orgs (including iProjectX) will not be modified', v_org;
END
$resolve$;
...
-- ---------- B) Wipe iSafeX operational / project data ONLY ----------
DO $wipe_ts$
DECLARE
...
WHERE org_id = current_setting('isafex.seed_org_id')::uuid
  AND (kind ILIKE 'timesheet%' OR kind IN ('timesheet_missing', 'timesheet_approval_reminder'));

DELETE FROM public.resource_allocations WHERE org_id = current_setting('isafex.seed_org_id')::uuid;
DELETE FROM public.resources WHERE org_id = current_setting('isafex.seed_org_id')::uuid;
DELETE FROM public.demand_pipeline WHERE org_id = current_setting('isafex.seed_org_id')::uuid;
DELETE FROM public.financials_monthly WHERE org_id = current_setting('isafex.seed_org_id')::uuid;
DELETE FROM public.fy_allocations WHERE org_id = current_setting('isafex.seed_org_id')::uuid;
DELETE FROM public.benefits WHERE org_id = current_setting('isafex.seed_org_id')::uuid;
DELETE FROM public.status_updates WHERE org_id = current_setting('isafex.seed_org_id')::uuid;
DELETE FROM public.documents WHERE org_id = current_setting('isafex.seed_org_id')::uuid;
DELETE FROM public.lessons_learned WHERE org_id = current_setting('isafex.seed_org_id')::uuid;

DO $wipe_opex$
DECLARE v_org uuid := current_setting('isafex.seed_org_id')::uuid;
BEGIN
...
-- Cascades remaining children for iSafeX only
DELETE FROM public.projects WHERE org_id = current_setting('isafex.seed_org_id')::uuid;

-- ---------- C) Ensure stage gate definitions (canonical 9) for iSafeX ----------
...
WHERE o.id = current_setting('isafex.seed_org_id')::uuid
ON CONFLICT (org_id, gate_name) DO UPDATE
SET sort_order = EXCLUDED.sort_order, is_active = true;

-- ---------- D) Seed 3 projects + streams + full attribute data ----------
DO $$
DECLARE
  r_org RECORD;
  r_bu uuid;
  ...
  i int;
  j int;
  codes text[] := ARRAY['SAFE-001','SAFE-002','SAFE-003'];
  names text[] := ARRAY[
    'Permit-to-Work Digitization','Contractor Induction Portal',
    'Incident Reporting & Analytics'
  ];
  programs text[] := ARRAY['Safety Systems','Field Enablement','Compliance Analytics'];
  phases text[] := ARRAY['Build','Design','Discovery'];
  statuses public.project_status[] := ARRAY['In Progress','In Progress','Not Started'];
  rags public.project_rag[] := ARRAY['Green','Amber','Green'];
  priorities text[] := ARRAY['P1 - Critical','P2 - High','P1 - Critical'];
  methods public.delivery_method[] := ARRAY['Hybrid','Agile','Waterfall'];
  budgets numeric[] := ARRAY[850000,420000,650000];
  capex_a numeric[] := ARRAY[600000,280000,450000];
  capex_i numeric[] := ARRAY[220000,95000,0];
  opex_a numeric[] := ARRAY[250000,140000,200000];
  opex_i numeric[] := ARRAY[90000,40000,0];
  facs numeric[] := ARRAY[880000,450000,650000];
  ben_t numeric[] := ARRAY[1600000,900000,1200000];
  ben_r numeric[] := ARRAY[320000,120000,0];
  rois numeric[] := ARRAY[88.2,114.3,84.6];
  starts date[] := ARRAY['2025-07-01','2025-10-01','2026-01-01'];
  ends date[] := ARRAY['2026-06-30','2026-09-30','2026-12-31'];
  lives date[] := ARRAY['2026-06-15','2026-09-15','2026-12-15'];
  sponsor_titles text[] := ARRAY['CSO','COO','CCO'];
  alt_names text[] := ARRAY['Permits','Induction','Analytics'];
  alt_codes text[] := ARRAY['PTW','IND','ANL'];
  align text[] := ARRAY['Risk Reduction','Field Enablement','Compliance'];
  ...
  sponsor_email text;
BEGIN
  FOR r_org IN SELECT id, COALESCE(fy_start_month, 4) AS fy_start_month FROM public.organizations WHERE id = current_setting('isafex.seed_org_id')::uuid LOOP
    ...
    FOR i IN 1..3 LOOP
      ...
      INSERT INTO public.projects (
        ...
        (ARRAY['Safety Strategic','Field Enablement','Compliance'])[((i - 1) % 3) + 1],
        ...
        'iSafeX sample project with Core + ' || alt_names[i] || ' streams for demo and training.',
        brief_json
    ...
    INSERT INTO public.demand_pipeline (
      ...
      (r_org.id, r_bu, 'Wearable lone-worker alerts', seed_person_names[1], 'Early demand idea for wearable panic / check-in', 'Idea', 480000, 900000, 55, 4, 2),
      (r_org.id, r_bu, 'Site access QR upgrade', seed_person_names[LEAST(2, n_people)], 'Under assessment for contractor gate access', 'Assessment', 320000, 650000, 40, 3, 2),
      (r_org.id, r_bu, 'Safety leading-indicator dashboard', seed_person_names[1], 'Business case for predictive safety KPIs', 'Business Case', 550000, 1100000, 60, 5, 3);

    -- Portfolio scenario
    INSERT INTO public.portfolio_scenarios (org_id, name, description, budget_cap, config)
    VALUES (
      r_org.id, 'FY safety portfolio', 'What-if: keep P1/P2 safety initiatives and defer low priority',
      2500000, '{"theme":"safety","include_amber":true}'::jsonb
...
-- ---------- E) Timesheets / assignees / PM links (iSafeX only) ----------
DO $$
DECLARE
  r_org RECORD;
  member_ids uuid[];
...
BEGIN
  ...
  FOR r_org IN SELECT id FROM public.organizations WHERE id = current_setting('isafex.seed_org_id')::uuid LOOP
...
-- Verification — iSafeX (expect 3 projects, 6 streams, ~54 gates)
SELECT o.name AS org, o.slug,
       ...
       (SELECT count(*) FROM public.decisions d WHERE d.org_id = o.id) AS decisions,
...
-- Sanity: iProjectX unchanged. iSafeX should show projects = 3.
