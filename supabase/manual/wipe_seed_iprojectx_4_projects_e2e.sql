...
-- RAID codes + sponsor sync (mirrors 20260816150000; idempotent)
...
BEGIN
  FOR r_org IN SELECT id, COALESCE(fy_start_month, 4) AS fy_start_month FROM public.organizations WHERE id = current_setting('iprojectx.seed_org_id')::uuid LOOP
    ...
    FOR i IN 1..4 LOOP
      ...
      SELECT r_org.id, p_id, sponsor_name, 'Executive Sponsor', sponsor_email, 'High', 'High', 'Monthly steering', true
      WHERE NOT EXISTS (
        SELECT 1 FROM public.stakeholders s
        WHERE s.project_id = p_id AND lower(btrim(s.name)) = lower(btrim(sponsor_name))
      );

      UPDATE public.stakeholders
      SET email = sponsor_email,
          role = COALESCE(NULLIF(btrim(role), ''), 'Executive Sponsor'),
          influence = 'High',
          interest = 'High',
          engagement_strategy = COALESCE(engagement_strategy, 'Monthly steering'),
          is_sponsor = true
      WHERE project_id = p_id AND lower(btrim(name)) = lower(btrim(sponsor_name));
