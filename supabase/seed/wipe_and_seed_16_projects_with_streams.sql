  ...
  FOR r_org IN SELECT id, COALESCE(fy_start_month, 4) AS fy_start_month FROM public.organizations LOOP
    ...
    FOR i IN 1..16 LOOP
      ...
      INSERT INTO public.status_updates (
        ...
        (r_org.id, p_id, CURRENT_DATE - 7, 'Sam Rivera', rags[i], rags[i], 'Green', 'Green',
        ...
        (r_org.id, p_id, CURRENT_DATE, 'Sam Rivera', rags[i],
         CASE WHEN rags[i] = 'Green' THEN 'Green' ELSE rags[i] END,
         CASE WHEN facs[i] > budgets[i] THEN 'Amber' ELSE 'Green' END,
         'Green',
