...
function Billing() {
  ...
  const { data: plans = [] } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("billing_plans").select("*").eq("active", true).order("sort_order"))
        .data ?? [],
  });
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-2xl">
              {(subscription as any)?.billing_plans?.name ?? "Free"}
            </CardTitle>
          </CardHeader>
          <CardContent className="text-xs text-muted-foreground">
            Status: <Badge variant="outline">{subscription?.status ?? "active"}</Badge>
            {subscription?.current_period_end && (
              <div className="mt-1">Renews {subscription.current_period_end}</div>
            )}
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-2xl text-green-700">{money(totalPaid)}</CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardTitle
              className={"text-2xl " + (totalDue > 0 ? "text-red-700" : "text-muted-foreground")}
            >
              {money(totalDue)}
            </CardTitle>
          </CardHeader>
                    ...
                    <td>
                      <span
                        className={
                          ...
                          (statusColor[i.status] ?? "bg-gray-100")
                        }
                      >
                        {i.status}
                      </span>
