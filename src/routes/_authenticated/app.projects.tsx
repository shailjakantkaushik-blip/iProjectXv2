...
import {
  ...
  LabelList, Legend, CartesianGrid,
} from "recharts";
...
function ProjectsList() {
  ...
  const programs = useMemo(() => Array.from(new Set(projects.map((p: any) => p.program).filter(Boolean))).sort() as string[], [projects]);
  const statuses = useMemo(() => Array.from(new Set(projects.map((p: any) => p.status).filter(Boolean))).sort() as string[], [projects]);
  ...
  const ragData = ["Green", "Amber", "Red"].map((r) => ({
    name: r, value: filtered.filter((p: any) => p.rag === r).length, color: RAG_COLORS[r],
  })).filter((d) => d.value > 0);
  ...
  const byProgram = useMemo(() => {
    ...
    filtered.forEach((p: any) => {
      ...
      cur.count += 1; cur.budget += Number(p.budget || 0);
      m.set(k, cur);
    });
  ...
  const byPriority = useMemo(() => {
    ...
    return Array.from(m, ([name, count]) => ({ name, count, color: PRIORITY_COLORS[name] || "#94a3b8" }))
      .sort((a, b) => rank(a.name) - rank(b.name));
  }, [filtered]);

  return (
    <PageExport name="Project_Register" title="Project Register">
      <PageHeading icon="📁" title="Project Register" subtitle="Full portfolio inventory with filters, KPIs & analytics."
      ...
      <SectionFrame>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Select value={prog} onValueChange={setProg}><SelectTrigger><SelectValue placeholder="Program" /></SelectTrigger>
          ...
          <Select value={ragF} onValueChange={setRagF}><SelectTrigger><SelectValue placeholder="RAG" /></SelectTrigger>
            <SelectContent><SelectItem value="All">All RAG</SelectItem><SelectItem value="Green">Green</SelectItem><SelectItem value="Amber">Amber</SelectItem><SelectItem value="Red">Red</SelectItem></SelectContent>
          </Select>
          <Select value={statusF} onValueChange={setStatusF}><SelectTrigger><SelectValue placeholder="Status" /></SelectTrigger>
      ...
      <SectionFrame>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          ...
          <KpiCard label="Utilisation" value={`${utilPct}%`} sub={money(capexIncurred)} accent="#ec4899" />
        </div>
      </SectionFrame>

      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <SectionFrame>
          <SectionTitle>Portfolio Health (RAG)</SectionTitle>
          <div className="h-56">
            <ResponsiveContainer>
              <PieChart>
                <Pie data={ragData} dataKey="value" nameKey="name" innerRadius={40} outerRadius={75} paddingAngle={2}>
        ...
        <SectionFrame className="lg:col-span-2">
          <SectionTitle>Budget by Program</SectionTitle>
          <div className="h-56">
            <ResponsiveContainer>
              <BarChart data={byProgram} margin={{ top: 15, right: 10, left: 0, bottom: 5 }}>
                ...
                <Bar dataKey="budget" radius={[4, 4, 0, 0]}>
                  {byProgram.map((_, i) => <Cell key={i} fill={PROGRAM_COLORS[i % PROGRAM_COLORS.length]} />)}
                  <LabelList dataKey="budget" position="top" formatter={(v: number) => money(v)} style={{ fontSize: 10, fill: "#475569" }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
      ...
      <SectionFrame>
        ...
        <div className="h-48">
          <ResponsiveContainer>
            <BarChart data={byPriority} layout="vertical" margin={{ top: 5, right: 30, left: 60, bottom: 5 }}>
      ...
      <SectionFrame>
        ...
        {isLoading ? (
          ...
          <div className="p-12 text-center text-sm text-muted-foreground">
            No projects match. {admin ? "Import from Excel or click New." : "Ask your admin to add projects."}
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="st-table">
              <thead>
                <tr>
                  <th>Project ID</th><th>Project Name</th><th>Program</th><th>Sponsor</th>
                  <th>Priority</th><th>Status</th><th>RAG</th><th>Current Phase</th>
                  <th>Method</th><th className="text-right">Budget</th><th className="text-right">Incurred</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((p: any) => (
                  <tr key={p.id}>
                    ...
                    <td><EditableCell table="projects" rowId={p.id} field="program" value={p.program} invalidateKeys={["projects"]} /></td>
                    <td><EditableCell table="projects" rowId={p.id} field="sponsor" value={p.sponsor} invalidateKeys={["projects"]} /></td>
                    <td><EditableCell table="projects" rowId={p.id} field="priority" value={p.priority} type="select" options={[{label:"P1 - Critical",value:"P1 - Critical"},{label:"P2 - High",value:"P2 - High"},{label:"P3 - Medium",value:"P3 - Medium"},{label:"P4 - Low",value:"P4 - Low"}]} invalidateKeys={["projects"]} /></td>
                    <td><EditableCell table="projects" rowId={p.id} field="status" value={p.status} type="select" options={[{label:"Not Started",value:"Not Started"},{label:"In Progress",value:"In Progress"},{label:"On Hold",value:"On Hold"},{label:"Completed",value:"Completed"},{label:"Cancelled",value:"Cancelled"}]} invalidateKeys={["projects"]} /></td>
                    <td><EditableCell table="projects" rowId={p.id} field="rag" value={p.rag} type="select" options={[{label:"Green",value:"Green"},{label:"Amber",value:"Amber"},{label:"Red",value:"Red"}]} invalidateKeys={["projects"]} display={(v)=><RagChip rag={v} />} /></td>
                    <td><EditableCell table="projects" rowId={p.id} field="current_phase" value={p.current_phase} invalidateKeys={["projects"]} /></td>
                    <td><EditableCell table="projects" rowId={p.id} field="delivery_method" value={p.delivery_method} type="select" options={[{label:"Waterfall",value:"Waterfall"},{label:"Agile",value:"Agile"},{label:"Hybrid",value:"Hybrid"}]} invalidateKeys={["projects"]} /></td>
                    <td className="text-right tabular-nums"><EditableCell table="projects" rowId={p.id} field="budget" value={p.budget} type="number" invalidateKeys={["projects"]} display={(v)=>money(Number(v||0))} /></td>
                    <td className="text-right tabular-nums"><EditableCell table="projects" rowId={p.id} field="capex_incurred" value={p.capex_incurred} type="number" invalidateKeys={["projects"]} display={(v)=>money(Number(v||0))} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
