...
function InvoicesPage() {
  ...
  const [form, setForm] = useState<any>({ org_id: "", amount_cents: 0, notes: "", period_start: "", period_end: "" });

  const { data: invoices = [] } = useQuery({
    queryKey: ["platform_invoices"],
    queryFn: async () => (await supabase.from("invoices").select("*, organizations(name)").order("issue_date", { ascending: false })).data ?? [],
  });
  const { data: orgs = [] } = useQuery({
    queryKey: ["all_orgs"],
    queryFn: async () => (await supabase.from("organizations").select("id,name,billing_email").order("name")).data ?? [],
  });

  const emailFn = useServerFn(emailInvoice);
  const emailIt = useMutation({
    mutationFn: async (id: string) => emailFn({ data: { invoiceId: id } }),
    onSuccess: (r: any) => { toast.success(`Emailed to ${r.to}`); qc.invalidateQueries({ queryKey: ["platform_invoices"] }); },
  ...
  const updateBillingEmail = useMutation({
    mutationFn: async ({ orgId, email }: { orgId: string; email: string }) =>
      supabase.from("organizations").update({ billing_email: email || null }).eq("id", orgId),
    onSuccess: () => { toast.success("Billing email updated"); qc.invalidateQueries({ queryKey: ["all_orgs"] }); },
  });
  ...
  const create = useMutation({
    ...
    onSuccess: () => { toast.success("Invoice sent"); qc.invalidateQueries({ queryKey: ["platform_invoices"] }); setCreating(false); },
  ...
  const setStatus = useMutation({
    ...
    onSuccess: () => { toast.success("Updated"); qc.invalidateQueries({ queryKey: ["platform_invoices"] }); },
  });

  const markPaid = useMutation({
    mutationFn: async (inv: any) => {
      await supabase.from("invoice_payments").insert({ invoice_id: inv.id, amount_cents: inv.amount_cents, method: "manual" });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Send invoices to customer organizations and track payments.</p>
        </div>
        <Button onClick={() => setCreating(!creating)}><Plus className="mr-2 h-4 w-4" />New invoice</Button>
      </div>
      ...
      {creating && (
        <Card>
          ...
          <CardContent className="grid gap-3 md:grid-cols-2">
            ...
            <div className="md:col-span-2"><Button onClick={() => create.mutate()} disabled={!form.org_id || !form.amount_cents}>Send invoice</Button></div>
          </CardContent>
        </Card>
      )}

      <Card>
        <CardHeader><CardTitle>Billing emails per organization</CardTitle></CardHeader>
        <CardContent>
          <table className="st-table w-full">
            <thead><tr><th>Organization</th><th>Billing email (invoices go here)</th></tr></thead>
            <tbody>
              {orgs.map((o: any) => (
                <tr key={o.id}>
                  <td>{o.name}</td>
      ...
      <Card>
        <CardHeader><CardTitle>{invoices.length} invoices</CardTitle></CardHeader>
        <CardContent>
          <table className="st-table w-full">
            <thead><tr><th>Invoice</th><th>Organization</th><th>Issue</th><th>Due</th><th className="text-right">Amount</th><th>Status</th><th>Emailed</th><th className="text-right">Actions</th></tr></thead>
            <tbody>
              {invoices.map((i: any) => (
                <tr key={i.id}>
                  ...
                  <td className="text-xs text-muted-foreground">
                    {i.emailed_at ? new Date(i.emailed_at).toLocaleString() : (i.email_last_error ? <span className="text-red-600" title={i.email_last_error}>failed</span> : "—")}
                  </td>
                  <td className="text-right space-x-2">
                    <Button size="sm" variant="outline" disabled={emailIt.isPending} onClick={() => emailIt.mutate(i.id)}>
                      <Mail className="mr-1 h-3 w-3" />{i.emailed_at ? "Re-send" : "Email"}
                    </Button>
                    {i.status !== "paid" && <Button size="sm" variant="outline" onClick={() => markPaid.mutate(i)}><Check className="mr-1 h-3 w-3" />Mark paid</Button>}
                  </td>
                </tr>
              ))}
            </tbody>
