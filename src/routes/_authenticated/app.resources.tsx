...
type Resource = { id: string; name: string; role?: string | null; skills?: string | null; capacity_hours_week?: number | null };
type Allocation = {
  id: string; resource_id: string; project_id: string;
  period_month: string; allocation_percent: number | null;
};
...
function ResourcesPage() {
  ...
  const { data: allocationsAll = [] } = useQuery({
    ...
    queryFn: async () => (await supabase.from("resource_allocations").select("*")).data as Allocation[] ?? [],
    enabled: !!organization,
  });
  const { data: projects = [] } = useQuery({
    queryKey: ["projects-r", organization?.id],
    queryFn: async () => (await supabase.from("projects").select("id,name")).data as Project[] ?? [],
    enabled: !!organization,
  });
  ...
  const skillOptions = useMemo(() => {
    ...
    resourcesAll.forEach((r) => (r.skills || "").split(",").map((x) => x.trim()).filter(Boolean).forEach((x) => s.add(x)));
  ...
  const monthsInRange = useMemo(() => {
    return monthOptionsAll.filter((m) => (monthFrom === "all" || m >= monthFrom) && (monthTo === "all" || m <= monthTo));
  }, [monthOptionsAll, monthFrom, monthTo]);

  const resources = useMemo(() => {
    const q = search.trim().toLowerCase();
    return resourcesAll.filter((r) => {
      if (q && !`${r.name} ${r.role ?? ""} ${r.skills ?? ""}`.toLowerCase().includes(q)) return false;
      ...
      if (projectFilter !== "all") {
        const hasProj = allocationsAll.some((a) => a.resource_id === r.id && a.project_id === projectFilter);
  ...
  }, [resourcesAll, allocationsAll, search, roleFilter, skillFilter, projectFilter, statusFilter, monthsInRange]);
  ...
  void resById; void projById;

  const resetFilters = () => {
    setSearch(""); setRoleFilter("all"); setSkillFilter("all");
    setProjectFilter("all"); setStatusFilter("all"); setMonthFrom("all"); setMonthTo("all");
  };
  ...
  const heatGrid = useMemo(() => {
    return resources.map((r) => {
      const row: { name: string; cells: { month: string; pct: number }[] } = { name: r.name, cells: [] };
      months.forEach((m) => {
  ...
  const bySkill = useMemo(() => {
    ...
    allocations.forEach((a) => {
      ...
      const skills = (r?.skills || r?.role || "Unknown").split(",").map((s) => s.trim()).filter(Boolean);
      const share = Number(a.allocation_percent || 0) / (skills.length || 1);
      skills.forEach((s) => m.set(s, (m.get(s) || 0) + share));
    });
    return Array.from(m.entries()).map(([skill, pct]) => ({ skill, pct: Math.round(pct) }))
  ...
  return (
    <PageExport name="Resource_Capacity" title="Resource Capacity & Skill Intelligence">
      ...
      <SectionFrame>
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 items-end">
          <div>
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Name, role, skill…" className="h-9" />
          </div>
          <div>
          ...
          <div>
            ...
            <Select value={monthFrom} onValueChange={setMonthFrom}>
              ...
              <SelectContent>
                ...
                {monthOptionsAll.map((m) => <SelectItem key={m} value={m}>{monthLabel(m)}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
      ...
      <SectionFrame>
        <SectionTitle>Resource Utilisation (avg monthly allocation)</SectionTitle>
        <div className="h-80">
          <ResponsiveContainer>
            <BarChart data={utilisation} margin={{ top: 20, right: 60, left: 20, bottom: 60 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(11,18,32,0.08)" />
              <XAxis dataKey="resource" fontSize={11} angle={-25} textAnchor="end" interval={0} height={60}
                     label={{ value: "Resource", position: "insideBottom", offset: -50, fontSize: 11 }} />
              <YAxis fontSize={11} domain={[0, 120]} label={{ value: "Allocation %", angle: -90, position: "insideLeft", fontSize: 11 }} />
              <Tooltip formatter={(v: number) => `${v}%`} />
              <Legend verticalAlign="top" wrapperStyle={{ fontSize: 11 }} />
              <ReferenceLine y={100} stroke="#dc2626" strokeDasharray="6 4"
        ...
        <div className="mt-1 flex justify-end gap-3 text-xs">
          <span className="flex items-center gap-1"><span className="inline-block h-3 w-3 rounded-sm" style={{ background: STATUS_COLOR.Over }} /> Over</span>
          <span className="flex items-center gap-1"><span className="inline-block h-3 w-3 rounded-sm" style={{ background: STATUS_COLOR.Optimal }} /> Optimal</span>
          <span className="flex items-center gap-1"><span className="inline-block h-3 w-3 rounded-sm" style={{ background: STATUS_COLOR.Under }} /> Under</span>
        </div>
      </SectionFrame>
      ...
      <SectionFrame>
          ...
          <table className="border-separate border-spacing-0 text-xs">
            <thead>
              <tr>
                ...
                {months.map((m) => (
                  <th key={m} className="px-2 py-1 text-center font-normal text-muted-foreground">{monthLabel(m)}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {heatGrid.map((row) => (
                <tr key={row.name}>
                  <td className="sticky left-0 z-10 bg-background px-2 py-1 font-medium">{row.name}</td>
                  {row.cells.map((c, i) => (
                    <td key={i} className="p-0">
                      <div
                        className="mx-0.5 my-0.5 flex h-8 min-w-[64px] items-center justify-center rounded text-[11px] font-semibold text-white"
                        style={{ background: c.pct === 0 ? "rgba(148,163,184,0.25)" : heatColor(c.pct), color: c.pct === 0 ? "#64748b" : "#fff" }}
        ...
        <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
          ...
          <div className="h-2 flex-1 rounded"
               style={{ background: "linear-gradient(to right, rgb(22,163,74), rgb(234,179,8), rgb(220,38,38))" }} />
          <span>120%</span>
        </div>
      </SectionFrame>

      <SectionFrame>
        <SectionTitle>Total Monthly Demand by Project</SectionTitle>
        <div className="h-80">
          <ResponsiveContainer>
            <BarChart data={monthlyByProject.data} margin={{ top: 10, right: 20, left: 20, bottom: 20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(11,18,32,0.08)" />
              <XAxis dataKey="month" fontSize={11} />
              <YAxis fontSize={11} label={{ value: "Allocation %", angle: -90, position: "insideLeft", fontSize: 11 }} />
              <Tooltip />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              {monthlyByProject.projList.map((p, i) => (
                <Bar key={p} dataKey={p} stackId="d" fill={["#1d4ed8", "#0ea5e9", "#8b5cf6", "#ec4899", "#f59e0b", "#10b981", "#6366f1"][i % 7]} />
              ))}
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <SectionFrame>
          <SectionTitle>Demand by Skill</SectionTitle>
          <div className="h-72">
            <ResponsiveContainer>
              <BarChart data={bySkill} margin={{ top: 10, right: 10, left: 10, bottom: 40 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(11,18,32,0.08)" />
                <XAxis dataKey="skill" fontSize={10} angle={-25} textAnchor="end" interval={0} height={50} />
                <YAxis fontSize={11} label={{ value: "Allocation %", angle: -90, position: "insideLeft", fontSize: 11 }} />
                <Tooltip formatter={(v: number) => `${v}%`} />
                <Bar dataKey="pct" fill="#0d9488" radius={[4, 4, 0, 0]}>
                  <LabelList dataKey="pct" position="top" fontSize={10} formatter={(v: number) => `${v}%`} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
              ...
              <thead>
                <tr>
                  ...
                  {rpGrid.projList.map((p) => (
                    <th key={p} className="px-2 py-1 text-center font-normal text-muted-foreground max-w-[120px] truncate" title={p}>{p}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                    ...
                    {row.cells.map((c, i) => (
                      <td key={i} className="p-0">
                        <div
                          ...
                          style={{
                            background: c.pct === 0 ? "rgba(148,163,184,0.2)" : heatColor(Math.min(100, c.pct / 3)),
      ...
      <SectionFrame>
        ...
        <div className="overflow-auto max-h-[420px]">
          <table className="st-table">
            <thead>
              <tr><th>Resource</th><th className="text-right">Allocation %</th><th>Status</th></tr>
            </thead>
            <tbody>
              {utilisation.map((u) => (
                <tr key={u.resource}>
                  ...
                  <td>
                    <span className="inline-block rounded-full px-2 py-0.5 text-xs font-medium text-white"
                          style={{ background: STATUS_COLOR[u.status] }}>{u.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="overflow-auto max-h-[420px]">
          <table className="st-table">
            <thead>
              <tr>
                <th>Resource</th>
                {months.map((m) => <th key={m} className="text-right">{m.slice(0, 7)}</th>)}
              </tr>
            </thead>
            <tbody>
              {heatGrid.map((row) => (
                <tr key={row.name}>
                  ...
                  {row.cells.map((c, i) => (
                    <td key={i} className="text-right tabular-nums">{c.pct}</td>
                  ))}
                </tr>
              ))}
            </tbody>
