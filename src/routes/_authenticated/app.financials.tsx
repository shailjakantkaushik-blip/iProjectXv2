...
function FinancialsPage() {
  ...
  const { data: monthly = [] } = useQuery({
    ...
    queryFn: async () => (await supabase.from("financials_monthly").select("*").order("period_month")).data ?? [],
    enabled: !!organization,
  });
  ...
  const mFiltered = useMemo(() => monthly.filter((m: any) => ids.has(m.project_id)), [monthly, ids]);
  ...
  const byProgram = Array.from(
    filtered.reduce((m: Map<string, any>, p: any) => {
      const k = p.program || "Unassigned";
      const cur = m.get(k) || { program: k, capex: 0, opex: 0, incurred: 0, budget: 0, benefits: 0 };
  ...
  const monthlyAgg = useMemo(() => {
    ...
    let cp = 0, ca = 0;
    return rows.map((r) => { cp += r.planned; ca += r.actual; return { ...r, cumPlanned: cp, cumActual: ca }; });
  }, [mFiltered]);

  // Top 10 variance (approved-incurred)
  const varianceTop = [...filtered]
    .map((p: any) => ({
      code: p.project_code, name: p.name,
      variance: Number(p.capex_approved || 0) + Number(p.opex_approved || 0) - Number(p.capex_incurred || 0) - Number(p.opex_incurred || 0),
    }))
    ...
    <PageExport name="Financials" title="Financial Intelligence">
      ...
      <SectionFrame>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-8">
          ...
          <KpiCard label="Spend %" value={`${spendPct.toFixed(1)}%`} accent={spendPct > 100 ? "#ef4444" : spendPct > 85 ? "#f59e0b" : "#22c55e"} />
          <KpiCard label="Benefits / Cost (CPI)" value={cpi.toFixed(2)} sub={`Variance ${money(variance)}`} accent="#0ea5e9" />
        </div>
      </SectionFrame>

      <div className="grid gap-4 lg:grid-cols-2">
        <SectionFrame>
          <SectionTitle>CAPEX vs OPEX vs Incurred by Program</SectionTitle>
          <div className="h-72">
            <ResponsiveContainer>
        ...
        <SectionFrame>
          <SectionTitle>Top 10 Budget Variance (Approved − Incurred)</SectionTitle>
          <div className="h-72">
            <ResponsiveContainer>
              <BarChart data={varianceTop} layout="vertical" margin={{ top: 5, right: 30, left: 60, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(11,18,32,0.08)" />
                ...
                <Bar dataKey="variance">
                  {varianceTop.map((v, i) => <Cell key={i} fill={v.variance < 0 ? "#ef4444" : "#22c55e"} />)}
                  <LabelList dataKey="variance" position="right" formatter={(x: number) => money(Number(x))} style={{ fontSize: 10, fill: "#334155" }} />
                </Bar>
      ...
      <SectionFrame>
        <SectionTitle>Monthly Cashflow — Planned vs Actual vs Forecast</SectionTitle>
        {monthlyAgg.length === 0 ? (
          <div className="p-6 text-center text-sm text-muted-foreground">No monthly financial data yet.</div>
        ) : (
          <div className="h-80">
            <ResponsiveContainer>
      ...
      <SectionFrame>
        <SectionTitle>Cumulative Cashflow (S-curve)</SectionTitle>
        {monthlyAgg.length === 0 ? (
          <div className="p-6 text-center text-sm text-muted-foreground">No monthly financial data.</div>
        ) : (
          <div className="h-72">
            <ResponsiveContainer>
              <AreaChart data={monthlyAgg}>
                ...
                <Tooltip formatter={(v: any) => money(Number(v))} />
                <Legend verticalAlign="top" height={28} wrapperStyle={{ fontSize: 11 }} />
                <Area type="monotone" dataKey="cumPlanned" stroke="#8b5cf6" fill="#c4b5fd" name="Cum. Planned" />
                <Area type="monotone" dataKey="cumActual" stroke="#1d4ed8" fill="#93c5fd" name="Cum. Actual" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        )}
              ...
              {filtered.map((p: any) => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td className={"text-right tabular-nums " + (vari < 0 ? "text-red-700" : "text-green-700")}>{money(vari)}</td>
                    <td className={"text-right tabular-nums " + (roi >= 0 ? "text-green-700" : "text-red-700")}>{roi.toFixed(1)}%</td>
                  </tr>
                );
              })}
            </tbody>
