...
export const Route = createFileRoute("/_authenticated/app/risk-roadmap")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Time-phased view of portfolio risks by severity and target resolution." },
    ],
  }),
  component: RiskRoadmapPage,
});

const SEV_COLOR = (s: number) => (s >= 15 ? "#dc2626" : s >= 9 ? "#f59e0b" : s >= 4 ? "#eab308" : "#22c55e");
const SEV_LABEL = (s: number) => (s >= 15 ? "Critical" : s >= 9 ? "High" : s >= 4 ? "Medium" : "Low");
...
function RiskRoadmapPage() {
  ...
  const overdue = risksFiltered.filter((r: any) => r.due_date && new Date(r.due_date) < new Date() && (r.status || "Open") !== "Closed").length;

  // Time-phased buckets by target resolution month
  const timeline = useMemo(() => {
    const buckets = new Map<string, { month: string; Critical: number; High: number; Medium: number; Low: number }>();
  ...
  const scatter = risksFiltered.map((r: any) => ({
    p: Number(r.probability || 1), i: Number(r.impact || 1), sev: Number(r.severity || 0), title: r.title,
  ...
  const byProject = useMemo(() => {
    ...
    for (const r of risksFiltered) {
      ...
      const row = m.get(key) || { code: key, name: proj?.name ?? "—", Critical: 0, High: 0, Medium: 0, Low: 0, total: 0 };
  ...
  return (
    <PageExport name="Risk_Roadmap" title="Risk Roadmap">
      ...
      <div className="text-sm text-muted-foreground mb-3">Time-phased portfolio risk view — target resolution timing, severity heat &amp; project rollup.</div>
      ...
      <div className="mb-3 flex flex-wrap gap-2 rounded-lg border bg-white/60 p-2">
        ...
        <select className="h-8 rounded-md border bg-white px-2 text-[12px]" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
          <option value="All">All statuses</option><option>Open</option><option>Mitigating</option><option>Closed</option>
        </select>
        <select className="h-8 rounded-md border bg-white px-2 text-[12px]" value={sevFilter} onChange={(e) => setSevFilter(e.target.value)}>
      ...
      <SectionFrame>
        <SectionTitle>Roadmap — Risks by Target Resolution Month</SectionTitle>
        {timeline.length === 0 ? (
          <div className="p-8 text-center text-sm text-muted-foreground">No risks with target dates match current filters.</div>
        ) : (
          <div className="h-72">
            <ResponsiveContainer>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
            ...
            <table className="border-collapse text-[12px]">
              <thead>
                <tr>
                  ...
                  {[1, 2, 3, 4, 5].map((i) => <th key={i} className="p-1 text-center w-10">{i}</th>)}
                </tr>
              </thead>
              <tbody>
                {[5, 4, 3, 2, 1].map((p) => (
                    ...
                    {[1, 2, 3, 4, 5].map((i) => {
                      ...
                      return (
                        <td key={i} className="p-0">
                          <div className="m-0.5 flex h-10 w-10 items-center justify-center rounded text-white font-semibold"
        ...
        <SectionFrame>
          <SectionTitle>Risk Scatter (probability × impact)</SectionTitle>
          <div className="h-64">
            <ResponsiveContainer>
              <ScatterChart margin={{ top: 10, right: 20, bottom: 20, left: 20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(11,18,32,0.08)" />
                <XAxis type="number" dataKey="p" domain={[0, 6]} name="Probability" fontSize={11} label={{ value: "Probability", position: "insideBottom", offset: -5, fontSize: 11 }} />
                <YAxis type="number" dataKey="i" domain={[0, 6]} name="Impact" fontSize={11} label={{ value: "Impact", angle: -90, position: "insideLeft", fontSize: 11 }} />
                <ZAxis type="number" range={[40, 160]} />
                <Tooltip cursor={{ strokeDasharray: "3 3" }} formatter={(v: any, k: string) => (k === "p" || k === "i" ? v : v)}
                  content={({ payload }) => {
                    const d: any = payload?.[0]?.payload; if (!d) return null;
                    return <div className="rounded border bg-white p-2 text-[11px] shadow">
                      <div className="font-semibold">{d.title}</div>
                      <div className="text-muted-foreground">{d.project}</div>
                      <div>Prob {d.p} • Impact {d.i} • Severity {d.sev}</div>
                    </div>;
                  }} />
                <Scatter data={scatter}>
                  {scatter.map((s, i) => <Cell key={i} fill={SEV_COLOR(s.sev)} />)}
                </Scatter>
              </ScatterChart>
            </ResponsiveContainer>
          </div>
      ...
      <SectionFrame>
        <SectionTitle>Risks per Project (top 15)</SectionTitle>
        {byProject.length === 0 ? (
          <div className="p-8 text-center text-sm text-muted-foreground">No risks match filters.</div>
        ) : (
          <div className="h-80">
            <ResponsiveContainer>
              <BarChart data={byProject} layout="vertical" margin={{ top: 5, right: 30, left: 60, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(11,18,32,0.08)" />
                ...
                <Bar dataKey="Medium" stackId="a" fill="#eab308" />
                <Bar dataKey="Low" stackId="a" fill="#22c55e">
                  <LabelList dataKey="total" position="right" style={{ fontSize: 10, fill: "#334155" }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <table className="st-table">
            <thead className="sticky top-0 bg-white">
              <tr>
                ...
                <th className="text-center">Prob</th><th className="text-center">Impact</th>
                <th className="text-center">Sev</th><th>Status</th><th>Due</th>
              </tr>
            </thead>
            <tbody>
              {risksFiltered.slice(0, 500).map((r: any) => {
                const proj = projectMap.get(r.project_id) as any;
                const sev = Number(r.severity || 0);
                const overdue = r.due_date && new Date(r.due_date) < new Date() && (r.status || "Open") !== "Closed";
                return (
                  <tr key={r.id}>
                    <td className="font-mono text-[11px]">
                      {proj?.project_code ? (
                        <Link to="/app/project-infographic" search={{ pid: proj.id }} className="text-primary hover:underline">{proj.project_code}</Link>
                      ) : "—"}
                    </td>
                    <td>{r.title}</td>
                    <td>{r.category || "—"}</td>
                    ...
                    <td className="text-center">
                      <span className="inline-flex items-center rounded px-2 py-0.5 text-[11px] font-semibold text-white"
                        style={{ background: SEV_COLOR(sev) }}>{sev} · {SEV_LABEL(sev)}</span>
                    </td>
                    <td>{r.status || "Open"}</td>
                    <td className={overdue ? "text-red-600 font-medium" : ""}>{r.due_date || "—"}</td>
                  </tr>
                );
              })}
            </tbody>
