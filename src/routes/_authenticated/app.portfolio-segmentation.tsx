...
import {
  ScatterChart, Scatter, XAxis, YAxis, ZAxis, Tooltip, CartesianGrid, ResponsiveContainer,
...
const PROG_COLORS = ["#3b82f6", "#8b5cf6", "#ec4899", "#22c55e", "#f59e0b", "#06b6d4", "#f43f5e", "#84cc16"];
...
function Segmentation() {
  ...
  const [dim, setDim] = useState<"program" | "priority" | "delivery_method" | "sponsor" | "current_phase">("program");
  ...
  return (
    <div>
      <PageHeading icon="🎨" title="Portfolio Segmentation" subtitle="Cluster projects by dimension to spot patterns and outliers." />
      ...
      <SectionFrame>
        <SectionTitle>Budget × ROI Scatter (bubble = benefits target)</SectionTitle>
        <div className="h-80">
          <ResponsiveContainer>
            <ScatterChart margin={{ top: 15, right: 20, left: 10, bottom: 20 }}>
              <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
              <XAxis type="number" dataKey="x" name="Budget" tickFormatter={money} tick={{ fontSize: 10 }} label={{ value: "Budget", position: "insideBottom", offset: -8, fontSize: 11 }} />
              <YAxis type="number" dataKey="y" name="ROI" unit="%" tick={{ fontSize: 10 }} label={{ value: "ROI %", angle: -90, position: "insideLeft", fontSize: 11 }} />
              ...
              <Tooltip
                ...
                formatter={(v: any, k: any) => k === "Budget" ? money(v) : `${Number(v).toFixed(1)}%`}
                labelFormatter={() => ""}
                content={({ payload }: any) => {
                  const p = payload?.[0]?.payload;
                  if (!p) return null;
                  return <div className="rounded border bg-background p-2 text-xs shadow"><div className="font-semibold">{p.name}</div><div>Budget: {money(p.x)}</div><div>ROI: {p.y?.toFixed(1)}%</div><div>Benefits target: {money(p.z)}</div></div>;
                }}
              />
              {["Green", "Amber", "Red"].map((r) => (
                <Scatter key={r} name={r} data={scatterData.filter((d) => d.rag === r)} fill={RAG[r]} />
              ))}
            </ScatterChart>
          </ResponsiveContainer>
        </div>
      ...
      <SectionFrame>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <BarChart data={segCounts} layout="vertical" margin={{ top: 5, right: 40, left: 80, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
                <XAxis type="number" tickFormatter={money} tick={{ fontSize: 10 }} />
                <YAxis type="category" dataKey="name" tick={{ fontSize: 10 }} width={80} />
                <Tooltip formatter={(v: number) => money(v)} />
                <Bar dataKey="budget" fill="#3b82f6">
                  <LabelList dataKey="budget" position="right" formatter={(v: number) => money(v)} style={{ fontSize: 10, fill: "#334155" }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
