...
const COLORS = ["#3b82f6", "#8b5cf6", "#ec4899", "#22c55e", "#f59e0b", "#06b6d4", "#f43f5e", "#84cc16"];
...
function ProgramsPage() {
  ...
  const programs = useMemo(() => {
    ...
    projects.forEach((p: any) => {
      ...
      const cur = m.get(k) || {
        name: k, count: 0, owner: "NA", sponsor: p.sponsor || "—", status: "Active",
        budget: 0, forecast: 0, approved: 0, actual: 0, fac: 0, benefits: 0,
        green: 0, amber: 0, red: 0, active: 0, completed: 0,
        startDates: [] as number[], endDates: [] as number[],
      };
      cur.count += 1;
      ...
      if (p.rag === "Green") cur.green++; else if (p.rag === "Amber") cur.amber++; else if (p.rag === "Red") cur.red++;
    ...
    return Array.from(m.values()).map((p) => ({
      ...p,
      startFY: p.startDates.length ? fyOf(new Date(Math.min(...p.startDates)), fyStartMonth) : "—",
  ...
  return (
    <PageExport name="Programs" title="Programs">
      <PageHeading icon="🎯" title="Programs" subtitle="Program-level rollups across the portfolio." />
      ...
      <SectionFrame>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <KpiCard label="Incurred" value={money(totActual)} sub={`${totBudget ? Math.round(totActual / totBudget * 100) : 0}% util`} accent="#f59e0b" />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <SectionFrame>
          <SectionTitle>Program Budget vs Committed vs Actual</SectionTitle>
          <div className="h-80">
            <ResponsiveContainer>
        ...
        <SectionFrame>
          <SectionTitle>Remaining Program Budget</SectionTitle>
          <div className="h-72">
            <ResponsiveContainer>
              <BarChart data={remainingSorted} layout="vertical" margin={{ top: 5, right: 60, left: 120, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
                <XAxis type="number" tickFormatter={money} tick={{ fontSize: 10 }} />
                <YAxis type="category" dataKey="name" tick={{ fontSize: 10 }} width={120} />
                <Tooltip formatter={(v: number) => money(v)} />
                <Bar dataKey="remaining" radius={[0, 4, 4, 0]}>
                  {remainingSorted.map((p, i) => <Cell key={i} fill={remColor(p.remaining)} />)}
                  <LabelList dataKey="remaining" position="right" formatter={(v: number) => money(v)} style={{ fontSize: 10, fill: "#334155" }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
      ...
      <SectionFrame>
        <SectionTitle>Portfolio waterfall — Budget → Committed → Actual → Remaining</SectionTitle>
        <div className="h-72">
          <ResponsiveContainer>
            <BarChart data={waterfall} margin={{ top: 20, right: 20, left: 10, bottom: 20 }}>
              ...
              <Bar dataKey="value" radius={[6, 6, 0, 0]}>
                ...
                <LabelList dataKey="value" position="top" formatter={(v: number) => moneyM(v)} style={{ fontSize: 11, fill: "#0f172a", fontWeight: 600 }} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      <SectionFrame>
        ...
        <div className="overflow-x-auto">
          <table className="st-table text-xs">
            <thead><tr>
              <th>Program</th><th>Owner</th><th>Sponsor</th><th>Status</th><th>Start FY</th><th>End FY</th>
            ...
            <tbody>
              {programs.map((p) => (
                <tr key={p.name}>
                  ...
                  <td>{p.startFY}</td>
                  <td>{p.endFY}</td>
                  <td className="text-right tabular-nums">{Math.round(p.budget).toLocaleString()}</td>
                  <td className="text-right tabular-nums">{Math.round(p.forecast).toLocaleString()}</td>
                  <td className="text-right tabular-nums">{Math.round(p.approved).toLocaleString()}</td>
                  <td className="text-right tabular-nums">{Math.round(p.actual).toLocaleString()}</td>
                  <td className="text-right tabular-nums">{Math.round(p.fac).toLocaleString()}</td>
                  <td className="text-right tabular-nums">{Math.round(p.committedVsBudget).toLocaleString()}</td>
                  <td className="text-right tabular-nums">{Math.round(p.remaining).toLocaleString()}</td>
                  <td className={`text-right tabular-nums ${p.variance < 0 ? "text-red-600" : "text-emerald-600"}`}>{Math.round(p.variance).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <p className="text-xs text-slate-500 mt-2">Showing {programs.length} of {programs.length} row(s). Click a column header to sort.</p>
      </SectionFrame>
        ...
        {currentProgram && (
          <>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <KpiCard label="Program Budget" value={moneyM(currentProgram.budget)} accent="#3b82f6" />
              <KpiCard label="Committed" value={moneyM(currentProgram.forecast)} accent="#22c55e" />
              <KpiCard label="Actual Spend" value={moneyM(currentProgram.actual)} accent="#f59e0b" />
              <KpiCard
                label="Remaining"
            ...
            <div className="mt-4">
              <p className="text-xs text-slate-600 mb-2">Projects mapped to <span className="font-semibold">{currentProgram.name}</span></p>
              <div className="overflow-x-auto">
                <table className="st-table text-xs">
                  <thead><tr>
                    <th>Project ID</th><th>Project Name</th><th>Sponsor</th><th>Status</th><th>RAG</th>
                  ...
                  <tbody>
                    {programProjects.map((p: any) => (
                      <tr key={p.id}>
                        <td className="font-mono text-blue-600">{p.project_id || p.id?.slice(0, 8)}</td>
                        <td>{p.name}</td>
                        <td>{p.sponsor || "—"}</td>
                        <td>{p.status || "—"}</td>
                        <td>{p.rag ? <RagChip rag={p.rag} /> : "—"}</td>
                        <td className="text-right tabular-nums">{Math.round(Number(p.approved_funding || 0)).toLocaleString()}</td>
                        <td className="text-right tabular-nums">{Math.round(Number(p.capex_incurred || 0)).toLocaleString()}</td>
                        <td className="text-right tabular-nums">{Math.round(Number(p.forecast_at_completion || p.forecast || p.budget || 0)).toLocaleString()}</td>
                        <td>{fmtDate(p.start_date)}</td>
                        <td>{fmtDate(p.end_date)}</td>
                      </tr>
                    ))}
                    {programProjects.length === 0 && (
                      <tr><td colSpan={10} className="text-center text-slate-500 py-4">No projects mapped.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              <p className="text-xs text-slate-500 mt-2">Showing {programProjects.length} of {programProjects.length} row(s).</p>
            </div>
          </>
        )}
      </SectionFrame>
      ...
      <SectionFrame>
        <SectionTitle>Program Health Distribution</SectionTitle>
        <div className="h-64">
          <ResponsiveContainer>
